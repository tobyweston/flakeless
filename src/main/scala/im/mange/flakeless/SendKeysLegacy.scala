...
object SendKeysLegacy {
  ...
  def apply(in: SearchContext, by: By, keysToSend: List[CharSequence], flakeless: Option[Flakeless] = None): Unit = {
    new SendKeysLegacy(flakeless, in, by, keysToSend).execute()
  }
}

private class SendKeysLegacy(flakeless: Option[Flakeless], in: SearchContext, by: By, keysToSend: List[CharSequence]) {
  def execute(): Unit = {
